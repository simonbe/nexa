function values = BinDataLoadSpecial(filename)
...
mformat
x = fread(fid, 2, 'int');%, 'ieee-le')
nrItems = x(1);
sizeItem = x(2);
nrMcs = fread(fid, sizeItem, 'int');
%values = [nrItems;sizeItem;nrMcs];
values = fread(fid, 1e9, 'float32');
