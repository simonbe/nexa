...
void GeometryUnit::SetPosition(float x, float y, float z)
{
	...
	if(this->m_unit->GetUnitType().compare("hypercolumn") == 0)
	{
		...
		for(int i=0;i<mcs.size();i++)
		{
			...
			// replace to get from connection instead
...
void TransferBcpnnOnline::Simulate(vector<UnitModifier*> events, vector<float> weights, Unit* unit)
{
	...
	for(map<long,float>::const_iterator it = hashHcIdSum.begin(); it != hashHcIdSum.end(); ++it)
	{
		...
		else if(it->second<-0.00001) // put this case here for the inhibitory recurrent connections (defined by impact weights -1.0 etc)
			m_value -= log(-it->second);
	}
...
void UnitModifierGraded::Send(int nodeId, long unitId)
{
	int tagAct = 0;
	MPI_Send(&unitId,1,MPI_LONG,nodeId,tagAct,NETWORK_COMM_WORLD); // id of sender (unit, not node)
	tagAct = 1;
	float value = this->GetEventData()[0];
	MPI_Send(&value,1,MPI_FLOAT,nodeId,tagAct,NETWORK_COMM_WORLD);
	tagAct = 2;
	MPI_Send(&m_fromHypercolumnId,1,MPI_INT,nodeId,tagAct,NETWORK_COMM_WORLD); // id of sender (hypercolumn)
}

void UnitModifierGraded::Receive(int nodeId)
{
	long unitId;
	float value;
	int tagAct = 0;
	...
	MPI_Recv(&unitId,1,MPI_LONG,nodeId,tagAct,NETWORK_COMM_WORLD,&status);
	tagAct = 1;
	MPI_Recv(&value,1,MPI_FLOAT,nodeId,tagAct,NETWORK_COMM_WORLD,&status);

	tagAct = 2;
	MPI_Recv(&m_fromHypercolumnId,1,MPI_INT,nodeId,tagAct,NETWORK_COMM_WORLD,&status);
